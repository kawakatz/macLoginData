...
import (
	...
	"github.com/kawakatz/macLoginData/pkg/utils"
...
var (
	...
	queryChromiumLoginData = `SELECT origin_url, username_value, password_value, date_created, date_last_used, date_password_modified FROM logins`
)
...
func ChromeLoginData(loginDataFile string, secretKey []byte, osType string) []types.LoginData {
	loginDataDB, _ := sql.Open("sqlite3", "file:"+loginDataFile)
	rows, _ := loginDataDB.Query(queryChromiumLoginData)

	var decryptedLoginData []types.LoginData
	for rows.Next() {
		var (
			originUrl, usernameValue                        string
			dateCreated, dateLastUsed, datePasswordModified int64
			password, passwordValue                         []byte
		)

		_ = rows.Scan(&originUrl, &usernameValue, &passwordValue, &dateCreated, &dateLastUsed, &datePasswordModified)

		loginData := types.LoginData{
			OriginURL:            originUrl,
			Username:             usernameValue,
			DateCreated:          utils.TimeEpochFormat(dateCreated),
			DateLastUsed:         utils.TimeEpochFormat(dateLastUsed),
			DatePasswordModified: utils.TimeEpochFormat(datePasswordModified),
		}

		if osType == "mac" {
			password, _ = decryptChromeAES(secretKey, passwordValue)
		} else {
			password, _ = decryptWindowsChrome(secretKey, passwordValue)
		}
		loginData.Password = string(password)

		decryptedLoginData = append(decryptedLoginData, loginData)
	}
